  ...
  generateBundle: async ({ dir }, bundle) => {
    ...
    for (const [fileName, chunk] of Object.entries(bundle)) {
      ...
      if (is_svelte) {
        ...
        if (css) {
          const clsname = snake(no_ext_name).replaceAll("_", "-"),
            name = "i-" + clsname;
          css = css
          ...
          let r = mincss(css, fileName, false);
          if (r === undefined) {
          ...
          r = mincss(css, fileName, true);
          if (r) {
            const { code } = r;
            css = code.toString();
          }
          write(join(dir_css, clsname + ".js"), css2mod(css));
          define = "S";
        } else {
          define = "C";
        }
        chunk.code =
          `import {${define}} from 'x/_.js'\n` +
          chunk.code.replace('customElements.define("i-', define + '("');
      }

      if (fileName.startsWith("custom-element")) {
        chunk.code =
          `import {DOC,Tag} from 'x/_.js'\n` +
          chunk.code
            .split("\n")
